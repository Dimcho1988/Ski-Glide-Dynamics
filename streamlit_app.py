...
# ---------------------------------------------------------
T_SEG = 12.0            # дължина на сегмента [s]
...
# -------------------- НОВО: ПЛАВАЩА СРЕДНА НА ВИСОЧИНАТА (САМО ТОВА Е ДОБАВЕНО) --------------------
def add_elev_smooth(points_df: pd.DataFrame, window_seconds: int) -> pd.DataFrame:
    """
    Добавя elev_smooth като time-based rolling mean (center=True) по активност.
    НЕ променя нищо друго.
    """
    df = points_df.copy()
...
def build_segments(df_activity, activity_label):
    ...
    # -------------------- ПРОМЯНА: ползваме elev_smooth ако я има --------------------
    if "elev_smooth" in df_activity.columns:
        elevs = df_activity["elev_smooth"].to_numpy()
    else:
...
# -------------------- НОВО: slider за ALT smoothing (САМО ТОВА Е ДОБАВЕНО) --------------------
ALT_SMOOTH_WINDOW_S = st.sidebar.slider(
    "Плаваща средна на височината (сек)",
    min_value=3,
    max_value=61,
    value=7,
    step=2,
    help="Time-based rolling mean върху AltitudeMeters (elev). Ползва се за slope."
)
# ---------------------------------------------------------------------------------------------
...
# -------------------- НОВО: прилагаме плаващата средна (САМО ТОВА Е ДОБАВЕНО) --------------------
points = add_elev_smooth(points, ALT_SMOOTH_WINDOW_S)
# --------------------------------------------------------------------------------------------------
