...
MIN_DOWNHILL_SEGMENTS = 10     # <-- променено от 20 на 10
...
MIN_SLOPE_SEGMENTS = 10        # <-- минимум сегменти за ΔV модела

# Модел 3: Зони спрямо критична скорост (CS)
DOWNHILL_RELAX_SLOPE = -5.0    # slope <= -5% -> спускане в Z1 (новият праг)
...
def parse_tcx(file) -> pd.DataFrame:
    ...
    (използваме простите сурови данни, както в работещия стар модел)
    """
    try:
        tree = ET.parse(file)
...
def build_segments(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Логиката е същата като в старото, работещо приложение.
    """
...
def filter_downhill_with_predecessor(seg_df: pd.DataFrame) -> pd.DataFrame:
    ...
    (както в старото приложение – това беше стабилно за всички активности,
    включително slow snow).
    """
    if seg_df.empty:
        return seg_df
...
def compute_glide_model(downhill_segments_list, raw_by_activity):
    ...
    - общ линеен модел V = a*slope + b (върху всички downhill сегменти),
    - Glide Index по активност,
    - модулирана обща скорост.
    Логиката е 1:1 със стария ти работещ код, само че MIN сегментите са 10.
    """
...
def show_ski_glide_view(a, b, down_trimmed, summary_df):
    """Визуализация за Модел 1 (същата логика като старото приложение)."""
...
def compute_slope_model(all_segments_glide: pd.DataFrame):
    ...
    (логиката е същата като в стария код, с праг MIN_SLOPE_SEGMENTS = 10)
    """
...
def show_slope_effect_view(v_flat, coeffs, all_corr: pd.DataFrame):
    """Визуализация + обобщителна таблица за Модел 2 (както в стария код)."""
...
def assign_zone(speed_eff_kmh: float, slope_percent: float, cs: float) -> str:
    ...
    - ако slope <= DOWNHILL_RELAX_SLOPE (-5%) -> спускане в Z1 (фиксирана горна граница);
...
def main():
    ...
    # 1) Модел за плъзгаемост (нужен и за следващите модели)
